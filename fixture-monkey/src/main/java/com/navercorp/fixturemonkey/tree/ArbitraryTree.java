...
public final class ArbitraryTree {
	...
	private ArbitraryGeneratorContext generateContext(
		DefaultArbitraryNode arbitraryNode,
		List<MatcherOperator<? extends FixtureCustomizer>> customizers,
		@Nullable ArbitraryGeneratorContext parentContext
	) {
		Map<ArbitraryProperty, DefaultArbitraryNode> childNodesByArbitraryProperty = new HashMap<>();
		...
		List<DefaultArbitraryNode> children = arbitraryNode.getChildren().stream()
			.filter(it -> resolvedParentProperty.equals(it.getResolvedParentProperty()))
			.collect(Collectors.toList());

		for (DefaultArbitraryNode childNode : children) {
		...
		return new ArbitraryGeneratorContext(
			...
			parentContext,
			(ctx, prop) -> {
				DefaultArbitraryNode node = childNodesByArbitraryProperty.get(prop);
				if (node == null) {
